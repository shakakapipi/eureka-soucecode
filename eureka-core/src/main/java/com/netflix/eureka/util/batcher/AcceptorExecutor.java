...
class AcceptorExecutor<ID, T> {
    ...
    AcceptorExecutor(String id,
        ...
        // 这里有一个 AcceptorRunner 线程任务 把 任务合并 networkFailureRetryMs 500ms 和 30s 取最小
        // maxBatchingSize= 250  最大batch 发送 250 个任务
        ...
        public void run() {
            ...
            while (!isShutdown.get()) {
                try {
                    ...
                    if (scheduleTime <= now) {
                        // BlockingQueue<List<TaskHolder<ID, T>>> batchWorkQueue 把任务 添加到 queue里
                        assignBatchWork();
