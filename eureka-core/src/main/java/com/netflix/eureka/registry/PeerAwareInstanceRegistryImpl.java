...
public class PeerAwareInstanceRegistryImpl extends AbstractInstanceRegistry implements PeerAwareInstanceRegistry {
    ...
    public void init(PeerEurekaNodes peerEurekaNodes) throws Exception {
        //开始每隔 60s 清空任务  清空一分钟实际心跳数量
        this.numberOfReplicationsLastMin.start();
        this.peerEurekaNodes = peerEurekaNodes;
        //初始化 eureka server 读写缓存  每180s自动过期   创建一个 30s任务 读写缓存和 只读缓存 数据比较 进行被动过期处理
        initializedResponseCache();
        // 15分钟任务 判断当前eureka server 服务实例 是否从别的服务获取到新的实例 如果有 重新计算期望每分钟心跳的数量
    ...
    public void openForTraffic(ApplicationInfoManager applicationInfoManager, int count) {
        ...
        //每次续约 心跳是 30s  eureka server 检测是否有宕机 是 60s  所以 * 2
        // 如果有20个服务实例   就是 20*2*0.85 = 期望心跳
     ...
     */
    //  服务 下线 通知 eureka server
    @Override
    public boolean cancel(final String appName, final String id,
                          final boolean isReplication) {
        if (super.cancel(appName, id, isReplication)) {

            // 服务下线了 通知eureka server 集群节点   isReplication true   接受 下线通知的 eureka server 只在本地执行
    ...
    public void register(final InstanceInfo info, final boolean isReplication) {
        ...
        // eureka server 接受 eureka client 注册
        super.register(info, leaseDuration, isReplication);
        //把新注册的信息 同步到集群  isReplication 是 false
    ...
    public boolean renew(final String appName, final String id, final boolean isReplication) {
        if (super.renew(appName, id, isReplication)) {
            ...
            // 向其他eureka server 发送续约 请求  isReplication 为true 接受续约eureka server 只在本地执行
    ...
    public boolean isLeaseExpirationEnabled() {
        ...
        //  numberOfRenewsPerMinThreshold =  expectedNumberOfRenewsPerMin（期望心跳的数量）* 0.85
        //  getNumOfRenewsInLastMin  实际一分钟心跳次数
    ...
    private void replicateToPeers(Action action, String appName, String id,
        ...
        //            isReplication  false  向其他 eureka server集群 发送 注册请求
        ...
        try {
            ...
            for (final PeerEurekaNode node : peerEurekaNodes.getPeerEurekaNodes()) {
                ...
                //  向每个eureka server 集群发送注册信息 此时 isReplication 设置为 ture 接受的eureka server 不需要向其他eureka server 注册
     ...
     *
     */
    // 想eureka server集群其他节点 发送 注册 下线的请求
