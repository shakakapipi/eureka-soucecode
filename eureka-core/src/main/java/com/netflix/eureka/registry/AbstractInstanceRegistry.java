...
public abstract class AbstractInstanceRegistry implements InstanceRegistry {
            ...
            if (existingLease != null && (existingLease.getHolder() != null)) {
                ...
                synchronized (lock) {
                    if (this.expectedNumberOfRenewsPerMin > 0) {
                        ...
                        // eureka server 每接受一个 eureka client 注册  期望心跳 + 2
     ...
     */
    //eureka server 获取 eureka client 服务续约 心跳
    public boolean renew(String appName, String id, boolean isReplication) {
        ...
        if (leaseToRenew == null) {
            ...
            //每次续约 的时候 会更新 这个 实际上一分钟心跳多少次 这个在eureka server 是否要自我保护机制用到了
            renewsLastMin.increment();
            // 更新 lastUpdateTimestamp 这个时间搓
            leaseToRenew.renew();
            return true;
        }
    }
    ...
    public void evict(long additionalLeaseMs) {
        ...
        // 先判断是否是自我保护机制 如果是自我保护状态 不能剔除如何服务
        ...
        if (toEvict > 0) {
            ...
            for (int i = 0; i < toEvict; i++) {
                ...
                //随机从 下线服务实例 找出3个下线操作  下次 在继续操作别的
    ...
    // eureka server 检查服务实例 是否宕机
    protected void postInit() {
        renewsLastMin.start();// 每次检查的时候 先把 心跳数量清零
        if (evictionTaskRef.get() != null) {
            evictionTaskRef.get().cancel();
        }
        evictionTaskRef.set(new EvictionTask());
        evictionTimer.schedule(evictionTaskRef.get(), //60s 检查一次 服务实例预期心跳和真是心跳
