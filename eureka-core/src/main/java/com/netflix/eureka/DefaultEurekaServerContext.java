...
public class DefaultEurekaServerContext implements EurekaServerContext {
    ...
    public void initialize() throws Exception {
        ...
        // 启动 eureka server 集群 , 更新eureka server 集群信息 ，然现在eureka server 感知别的eureka server集群
        // 然后搞一个定时任务 后台线程 每隔 一段时间 10 * 60 * 1000 （10分钟）  去维护 eureka server 集群信息
        //  创建了 eureka server 传输 三层队列机制
        peerEurekaNodes.start();
        //应该是 基于 eureka server 集群信息来初始化注册表   还有 初始化 eureka server 读写缓存信息 180s自动过期  和 每隔30s读写缓存 和只读缓存 做比较
